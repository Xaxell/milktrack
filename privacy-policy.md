# Privacy Policy

**Last updated:** 02-02-2026

## Introduction
This Privacy Policy explains how the MilkTrack app (“the app”) collects, uses, and protects user information. By using this app, you agree to the terms described in this policy.

## Information We Collect
The app collects only the information necessary to provide its core functionality, including:
- Milk subscription details (supplier name, milk brand,quantity, schedule,price, delivery time,user address)
- Delivery tracking information
- Payment records entered by the user
- Basic account information provided by the user
- Basic payment details of the supplier to make payments

The app does not collect sensitive personal data such as government-issued identification, biometric data.

## How We Use Information
The collected information is used solely to:
- Track daily milk deliveries
- Manage subscriptions
- Record payments
- Handle delivery adjustments and disputes

The data is not used for advertising, analytics, or marketing purposes.

## Data Sharing
We do not sell, rent, or share user data with third parties.  
Data may be disclosed only if required by law.

## Data Storage and Security
We take reasonable measures to protect user data against unauthorized access, alteration, or disclosure. Data is stored securely and used only for app functionality.

## User Rights and Data Deletion
Users may:
- Review or update their information within the app
- Request deletion of their data by contacting the developer

## Children’s Privacy
This app is not intended for use by children under the age of 13. We do not knowingly collect personal information from children.

## Changes to This Privacy Policy
This Privacy Policy may be updated from time to time. Any changes will be reflected on this page with an updated revision date.

## Contact Information
If you have questions about this Privacy Policy, please contact:

**Email:** [your-email@example.com]
